...
console.log('Challenge 1: ',fullName);
...
largeShirts = [];
noShirts = runners.filter(function(runner) {
    if(runner.shirt_size === "L")
        return largeShirts.push(`${runner.first_name} ${runner.last_name}`);
})
console.log('Challenge 3: ',largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = runners.reduce(function(total, x) {
...
// Problem 1 --- runner sick replacement #50
const newRunner = runners.filter(function(runner) {
    if(runner.id === 50)
        return {"id":50,"first_name":"x","last_name":"x","email":"x","shirt_size":"M","company_name":"Gabtype","donation":171};
})
runners[49] = newRunner;
console.log();

// Problem 2 --- Make a new array of nametags including first name, last name, and company.

nametags = runners.map(function(runner) {
    return `Hi, my name is ${runner.first_name} ${runner.last_name} and I am representing ${runner.company_name}!`;
})

console.log(nametags)

// Problem 3 --- Filter list of companies so you can see which companies donated $100+ and then sort by largest to smallest donations.

largestDonationsOrder = runners.filter(function(runner) {
    if(runner.donation >= 100)
        return runner.donation; 
})
largestDonationsOrder.sort(function(a,b) {b.donation - a.donation});

largestDonations = largestDonationsOrder.forEach(function(x) {
    return `${x.company_name} donated: ${x.donation}!`;
})

console.log('Challenge 5: ',largestDonations)
